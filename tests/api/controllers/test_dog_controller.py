...
class TestDogCongroller:
    ...
    def test_list_01(self):
        ...
        ユーザー、犬情報供に存在する場合
        """
        response = self.client.get(f"/owners/{OWNER_ID_1}/dogs/")
        body = response.json()
    ...
    def test_list_02(self):
        ...
        犬情報が登録されていないオーナーの場合
        """
        response = self.client.get(f"/owners/{OWNER_ID_2}/dogs/")
        body = response.json()
        assert response.status_code == 200
        assert len(body) == 0
    ...
    def test_list_03(self):
        ...
        オーナーが存在しない場合
        """
        response = self.client.get(f"/owners/{OWNER_ID_3}/dogs/")
        body = response.json()
        assert response.status_code == 200
        assert len(body) == 0
    ...
    def test_get_01(self):
        ...
        情報が取得できる場合
        """
        response = self.client.get(f"/owners/{OWNER_ID_1}/dogs/{DOG_ID_MIN}")
        body = response.json()
        assert response.status_code == 200
    ...
    def test_get_02(self):
        ...
        犬情報が存在しないIDが指定された場合
        """
        response = self.client.get(f"/owners/{OWNER_ID_1}/dogs/{DOG_ID_NOT_EXIST}")
        body = response.json()
    ...
    def test_get_03(self):
        ...
        """
        response = self.client.get(f"/owners/{OWNER_ID_3}/dogs/{DOG_ID_MIN}")
        body = response.json()
        assert response.status_code == 404
        assert body == {"detail": "dog not found."}
    ...
    def __create_model_min(self, owner_id: str, dog_id: str, updated_at: int):
        name = f"{dog_id}-name"
        return DogModel(
            owner_id=owner_id, dog_id=dog_id, name=name, updated_at=updated_at
        )
